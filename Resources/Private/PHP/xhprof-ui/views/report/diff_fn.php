...
?>
		<h2>Parent/Child <strong>diff</strong> report for <strong><?php echo $ui->fn;?></strong> (Run <a href="<?php echo $ui->url(array('fn' => null));?>">#<?php echo $ui->runs[0]->run_id;?></a>)</h3>
...
if ($ui->display_calls) {
	echo XHProf_UI\Utils::td_num($data2[$ui->fn]['ct'] - $data1[$ui->fn]['ct'], $ui->config->format_cbk['ct'], ($ui->sort == 'ct'));
	echo XHProf_UI\Utils::td_pct($data2[$ui->fn]['ct'] - $data1[$ui->fn]['ct'], $ui->totals['ct'], ($ui->sort == 'ct'));
}

foreach ($ui->metrics as $metric) {
	// Inclusive metric
	echo XHProf_UI\Utils::td_num($data2[$ui->fn][$metric] - $data1[$ui->fn][$metric], $ui->config->format_cbk[$metric], ($ui->sort == $metric));
	echo XHProf_UI\Utils::td_pct($data2[$ui->fn][$metric] - $data1[$ui->fn][$metric], $ui->totals[$metric], ($ui->sort == $metric));
}
